...
export async function GET() {
  ...
  if (!property) {
    return NextResponse.json({
      error: "GA4_PROPERTY_ID missing",
      segments: [] as { name: string; sessions: number }[],
    });
  }
  const auth = await getGoogleAccessToken([SCOPE]);
  if ("error" in auth) {
    return NextResponse.json({ error: auth.error, segments: [] });
  }
  ...
  try {
    ...
    const segments = (json.rows ?? []).map((row) => ({
