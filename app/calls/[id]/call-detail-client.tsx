...
const STATUS_COLOR: Record<string, string> = {
  ...
  Voicemail: "bg-amber-500/20 text-amber-700 ring-amber-500/30",
...
const ALIGN_COLOR: Record<string, string> = {
  ...
  matches_last_resort: "bg-amber-500/20 text-amber-700",
  deviated: "bg-rose-500/20 text-rose-300",
  missed: "bg-slate-500/20 text-slate-600",
};
...
export function CallDetailClient({ callId }: { callId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ---- Header card ---- */}
      <section className="rounded-xl border border-[#e2e8f0] p-6" style={{ backgroundColor: "#ffffff" }}>
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {c.transcription_language ? (
                <span className="inline-flex rounded-full bg-slate-500/20 px-2.5 py-0.5 text-xs uppercase text-slate-600 ring-1 ring-slate-500/30">
                  {c.transcription_language}
                </span>
              ) : null}
              {c.direction ? (
          ...
          <div className="flex flex-col items-end gap-3">
            {score?.overall_score != null ? (
              <div className="text-right">
                <p className="text-xs uppercase tracking-wide text-slate-500">Coach score</p>
        ...
        <>
          <section className="rounded-xl border border-[#e2e8f0] p-6" style={{ backgroundColor: "#ffffff" }}>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-900">Coach feedback</h2>
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-lg bg-[#ffffff] p-4 ring-1 ring-[#e2e8f0]">
                <p className="text-xs uppercase text-slate-500">Case type detected</p>
                <p className="mt-1 text-sm font-medium text-slate-900">{score.case_type_detected ?? "Unclear"}</p>
              </div>
              <div className="rounded-lg bg-[#ffffff] p-4 ring-1 ring-[#e2e8f0]">
                <p className="text-xs uppercase text-slate-500">Case quality</p>
                <p className="mt-1 text-sm font-medium text-slate-900">{score.case_quality_estimate ?? "—"}</p>
              </div>
              <div className="rounded-lg bg-[#ffffff] p-4 ring-1 ring-[#e2e8f0]">
                <p className="text-xs uppercase text-slate-500">Compliance flags</p>
                <p className="mt-1 text-sm font-medium text-slate-900">
                  {score.compliance_flags.length === 0 ? "None" : score.compliance_flags.length}
                </p>
              </div>
            </div>
            <div className="mt-4 rounded-lg bg-[#ffffff] p-4 ring-1 ring-[#e2e8f0]">
          ...
          {/* Rubric dimensions */}
          <section className="rounded-xl border border-[#e2e8f0] p-6" style={{ backgroundColor: "#ffffff" }}>
            <h2 className="mb-4 text-lg font-semibold text-slate-900">Rubric scores</h2>
            <div className="space-y-3">
              {score.dimension_scores.map((d) => {
                ...
                return (
                  <div key={d.dimension_key} className="rounded-lg bg-[#ffffff] p-4 ring-1 ring-[#e2e8f0]">
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-sm font-medium text-slate-900">{d.dimension_name}</p>
                    ...
                    {d.evidence ? (
                      <p className="mt-3 text-xs italic text-slate-600">&ldquo;{d.evidence}&rdquo;</p>
                    ) : null}
                    {d.missed ? (
          ...
          {score.objections_log.length > 0 ? (
            <section className="rounded-xl border border-[#e2e8f0] p-6" style={{ backgroundColor: "#ffffff" }}>
              <h2 className="mb-4 text-lg font-semibold text-slate-900">Objections handled</h2>
              <div className="space-y-3">
                {score.objections_log.map((o, i) => (
                  <div key={i} className="rounded-lg bg-[#ffffff] p-4 ring-1 ring-[#e2e8f0]">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <p className="text-sm font-medium text-slate-900">&ldquo;{o.objection}&rdquo;</p>
                      <span
                        className={`inline-flex rounded-full px-2.5 py-0.5 text-xs ${
                          ALIGN_COLOR[o.alignment] ?? "bg-slate-500/20 text-slate-600"
                        }`}
                      >
                    ...
                    {o.notes ? <p className="mt-2 text-xs text-slate-500">{o.notes}</p> : null}
                  </div>
                ))}
              </div>
            </section>
          ...
          {score.script_recommendations.length > 0 ? (
            <section className="rounded-xl border border-[#e2e8f0] p-6" style={{ backgroundColor: "#ffffff" }}>
              <h2 className="mb-4 text-lg font-semibold text-slate-900">Script recommendations</h2>
              <ul className="space-y-2 text-sm text-slate-700">
                {score.script_recommendations.map((s, i) => (
                  <li key={i} className="rounded-lg bg-[#ffffff] p-3 ring-1 ring-[#e2e8f0]">
                    {s}
                  </li>
                ))}
              </ul>
      ...
      <section className="rounded-xl border border-[#e2e8f0] p-6" style={{ backgroundColor: "#ffffff" }}>
        ...
        {c.transcription ? (
          <pre className="max-h-[500px] overflow-y-auto whitespace-pre-wrap rounded-lg bg-[#ffffff] p-4 text-sm text-slate-700 ring-1 ring-[#e2e8f0]">
            {c.transcription}
          </pre>
        ) : (
