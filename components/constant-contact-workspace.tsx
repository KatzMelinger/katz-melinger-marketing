...
/**
 * Constant Contact workspace — the Campaigns / Lists / Automation / Analytics
 * panels, extracted from the former standalone /constant-contact page so they
 * can render as tabs inside the unified /email dashboard. The active tab is
 * owned by the parent (/email) and passed in; this component renders only the
 * panel for that tab plus the shared OAuth/connection banners.
 */
...
/** OAuth round-trips return to /email (the workspace's home) now. */
const OAUTH_NEXT_PATH = "/email";

function buildOauthStartUrl(nextPath = OAUTH_NEXT_PATH): string {
  const params = new URLSearchParams({ next: nextPath });
  return `/api/constant-contact/oauth/start?${params.toString()}`;
}

function resolveAuthUrl(input: string | undefined): string {
  const fallback = buildOauthStartUrl(OAUTH_NEXT_PATH);
  if (!input?.trim()) return fallback;
  try {
    if (input.startsWith("http://") || input.startsWith("https://")) {
      const u = new URL(input);
      if (u.pathname !== "/api/constant-contact/oauth/start") return fallback;
      if (!u.searchParams.get("next")) u.searchParams.set("next", OAUTH_NEXT_PATH);
...
export type CcTabId = "campaigns" | "lists" | "automation" | "analytics";
...
export function ConstantContactWorkspace({ activeTab }: { activeTab: CcTabId }) {
  const [authActionUrl, setAuthActionUrl] = useState<string>(
    buildOauthStartUrl(OAUTH_NEXT_PATH),
  );
  ...
  const markUnauthorizedFromMessage = useCallback((message: string) => {
    ...
    setAuthActionUrl(buildOauthStartUrl(OAUTH_NEXT_PATH));
  }, []);
  ...
  return (
    <div className="space-y-8">
      <ConnectionHealthBadge className="max-w-md" />
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              onClick={() => {
                ...
                window.location.assign(authActionUrl || buildOauthStartUrl(OAUTH_NEXT_PATH));
              }}
