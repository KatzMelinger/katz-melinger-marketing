...
/**
 * Research Libraries + Research Layer — extracted from the former
 * /content/research page so it can be rendered both as a standalone page and
 * as a tab inside the Content Directions dashboard (app/brand-voice/page.tsx).
 *
 * Three tabs:
 ...
 * KM Brief Generator.
 *
 * Pass `showHeader={false}` when embedding under a host that already provides
 * its own page heading (the Content Directions tab does this).
 */
...
export function ResearchLibraries({ showHeader = true }: { showHeader?: boolean }) {
      ...
      {showHeader && (
        <header className="mb-6">
          ...
          <p className="mt-1 text-sm text-slate-600">
            ...
            the Research Layer to assemble a packet that feeds SEO content.
          </p>
        </header>
      )}
