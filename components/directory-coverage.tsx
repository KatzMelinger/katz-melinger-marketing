...
/**
 * Directory coverage — the "are we listed / claimed?" half of the unified
 * Directories & Citations page. Extracted from the old standalone
 * /seo/directories page so it can live as a section on /seo/citations (one page
 * answers both "are we listed?" and "is it accurate?"). Unchanged behavior:
 *   - Manual: add a directory, set its status / listing URL.
 *   - AI: "Suggest directories" asks Claude which directories matter for the
 *     firm's practice areas; one click adds each.
 *
 * Persists in seo_legal_directories via /api/seo/directories (unchanged).
 */

import { useEffect, useMemo, useState } from "react";

import { formatNumber } from "@/components/seo-shell";
  ...
  return (
    <section className="rounded-xl border border-[#e2e8f0] bg-white p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold">Directory coverage</h2>
          <p className="mt-1 text-xs text-slate-500">
            Are we listed and claimed everywhere we should be? Track legal + business directories,
            or let AI suggest the ones that matter for your practice areas. (Accuracy of each
            listing&apos;s NAP is tracked above.)
          </p>
        </div>
        <button
          onClick={suggest}
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-4">
        <div className="rounded-lg border border-[#e2e8f0] bg-slate-50 p-3">
          ...
          <p className="mt-1 text-xl font-semibold">{formatNumber(directories.length)}</p>
        </div>
        <div className="rounded-lg border border-[#e2e8f0] bg-slate-50 p-3">
          <p className="text-[11px] uppercase tracking-wide text-slate-500">Listed / claimed</p>
          <p className="mt-1 text-xl font-semibold text-emerald-700">{formatNumber(counts.done)}</p>
        </div>
        <div className="rounded-lg border border-[#e2e8f0] bg-slate-50 p-3">
          <p className="text-[11px] uppercase tracking-wide text-slate-500">Needs update</p>
          <p className="mt-1 text-xl font-semibold text-red-700">{formatNumber(counts.needsUpdate)}</p>
        </div>
        <div className="rounded-lg border border-[#e2e8f0] bg-slate-50 p-3">
          <p className="text-[11px] uppercase tracking-wide text-slate-500">Not listed</p>
          <p className="mt-1 text-xl font-semibold text-slate-500">{formatNumber(counts.notListed)}</p>
        </div>
      </div>

      {/* AI suggestions (a suggested-but-not-listed directory = coverage gap). */}
      {pendingSuggestions.length > 0 && (
        <ul className="mt-4 space-y-2">
          {pendingSuggestions.map((s) => (
            <li key={s.name} className="rounded-md border border-[#e2e8f0] bg-white p-3">
      ...
      <ul className="mt-4 space-y-2">
        {loading && directories.length === 0 && <li className="text-sm text-slate-500">Loading…</li>}
        {!loading && directories.length === 0 && (
          <li className="text-sm text-slate-500">
            No directories tracked yet. Click &ldquo;Suggest directories&rdquo; above to get started.
          </li>
        )}
        {sorted.map((d) => (
          <li
